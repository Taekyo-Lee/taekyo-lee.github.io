...
**Push with automatic retry**: The company network is flaky — the first `git
push` often fails with a connection error. It typically succeeds on the second
attempt. Always retry once before reporting a failure to the user.

```bash
git push origin main || git push origin main
```

If the second attempt also fails, then inform the user about the network issue
and suggest checking VPN connectivity.
